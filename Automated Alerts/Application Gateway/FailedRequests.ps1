...
##################  RESOURCES ###########################################################################################
#Get Resources
$VMColl = Get-AzResource -ResourceType "Microsoft.Network/applicationGateways" | Select-Object -Property ResourceId,Name,Tags
######################################################################################################################### 
...
if (!$VMID.Tags.FailedRequests)
{

$metricName = "FailedRequests"
$alertName = $strVMName + " - " + $metricName 
$metricNamespace = "Microsoft.Network"
$threshold = "100"
$actionGroupId = $alertgroup.Id
$timeAggregation = "Total" # Average, Minimum, Maximum, Total
$alertDescription = ""
...
$windowsSize = "PT5M"
    ...
    $alertName = $strVMName + " - " + $metricName 
    $metricNamespace = "Microsoft.Network"
    $threshold = $VMID.Tags.FailedRequests
    $actionGroupId = $alertgroup.Id
    $timeAggregation = "Total" # Average, Minimum, Maximum, Total
    $alertDescription = ""
    $operator = "GreaterThan" # Equals, NotEquals, GreaterThan, GreaterThanOrEqual, LessThan, LessThanOrEqual
