...
##################  RESOURCES ###########################################################################################
#Get Resources
$VMColl = Get-AzResource -ResourceType "Microsoft.ServiceBus/namespaces" | Select-Object -Property ResourceId,Name,Tags
######################################################################################################################### 
...
$alertName = $strVMName + " - " + $metricName 
$metricNamespace = "Microsoft.ServiceBus"
$threshold = $VMID.Tags.ServerErrors
$actionGroupId = $alertgroup.Id
$timeAggregation = "Total" # Average, Minimum, Maximum, Total
$alertDescription = ""
$operator = "GreaterThan" # Equals, NotEquals, GreaterThan, GreaterThanOrEqual, LessThan, LessThanOrEqual
